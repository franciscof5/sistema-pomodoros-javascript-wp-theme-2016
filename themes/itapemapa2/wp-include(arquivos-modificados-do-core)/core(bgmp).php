...
if( !class_exists( 'BasicGoogleMapsPlacemarks' ) )
{
	...
	class BasicGoogleMapsPlacemarks
	{
		...
		//const TAXONOMY		= 'bgmp-category'; f5sites
		...
		protected function cleanMapShortcodeArguments( $arguments )
		{
			...
			if( isset( $arguments[ 'width' ] ) )
			{
				...
				/*else 100% IS A VALID WIDTH DAMMIT f5sites
				{
					$this->enqueueMessage( sprintf(
		...
		public function loadResources()
		{
			...
			wp_register_script(
				'googleMapsAPI',
				'http'. ( is_ssl() ? 's' : '' ) .'://maps.google.com/maps/api/js?sensor=false&language=pt_BR&libraries=places' . $googleMapsLanguage,
				array(),
				false,
				true
			);
		...
		public function createPostType()
		{
			...
			if( !post_type_exists( self::POST_TYPE ) )
			{
				$labels = array(
					...
					'not_found'				=> __( 'Ainda não temos nenhum local nessa lista', 'bgmp' ),
		...
		public function listShortcode( $attributes )
		{
			...
			if( $posts )
			{
				...
				return '<p style="margin-left:22px;">Ainda não temos nenhum local nessa lista</p>';//f5sites francisco__( 'No Placemarks found', 'bgmp' );
		}
		...
		public function getMapPlacemarks( $attributes )
		{
			...
			if(isset($tipo_assinatura_filtrada)) {
				$query = array( 
					'numberposts'	=> -1,
					'post_type'		=> self::POST_TYPE,
					'post_status'	=> 'publish',
					'meta_key' 		=> 'tipo_assinatura',
					'meta_value'	=> $tipo_assinatura_filtrada
				);	
			} else {
				$query = array( 
					'numberposts'	=> -1,
