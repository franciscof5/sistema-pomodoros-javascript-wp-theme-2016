            ...
            switch ( $maybe_subscription->get_type() ) {
                case 'variation':
                    if ( ! empty( $variation_data[ 'sumosubs_plan_message' ] ) && self::is_subscription_synced( $maybe_subscription ) ) {
                        $variation_data[ 'sumosubs_synced_next_payment_date' ] = sprintf( '<p id="sumosubs_initial_synced_payment_date">%s<strong>%s</strong></p>', __( 'Next Payment on: ', 'sumosubscriptions' ), self::get_initial_payment_date( $maybe_subscription->get_id(), true ) ) ;
                    }
                    break ;
                case 'grouped':
