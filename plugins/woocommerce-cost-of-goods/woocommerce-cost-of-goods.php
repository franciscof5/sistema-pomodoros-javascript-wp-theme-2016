...
SV_WC_Framework_Bootstrap::instance()->register_plugin( '4.2.0', __( 'WooCommerce Costs of Goods', 'woocommerce-cost-of-goods' ), __FILE__, 'init_woocommerce_cost_of_goods', array(
	'minimum_wc_version'   => '2.3.6',
	'backwards_compatible' => '4.2.0',
) );
...
class WC_COG extends SV_WC_Plugin {
	...
	/**
	 * Backwards compat for changing the visibility of some class instances. This
	 * can be removed as part of WC 2.6 compatibility (assuming it's July/August 2016)
	 *
	 * @since 2.0.0
	 */
	public function __get( $name ) {
		...
		switch ( $name ) {
			...
			case 'admin':
				_deprecated_function( 'wc_cog()->admin', self::VERSION, 'wc_cog()->get_admin_instance()' );
				return $this->get_admin_instance();

			case 'reports':
				_deprecated_function( 'wc_cog()->reports', self::VERSION, 'wc_cog()->get_admin_reports_instance()' );
				return $this->get_admin_reports_instance();

			case 'import_export_handler':
				_deprecated_function( 'wc_cog()->import_export_handler', self::VERSION, 'wc_cog()->get_import_export_handler_instance()' );
