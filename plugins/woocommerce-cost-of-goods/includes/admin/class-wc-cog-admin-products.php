...
class WC_COG_Admin_Products {
	...
	protected function init_hooks() {
		...
		// save variation cost for bulk edit action - WC 2.4+
		...
		if ( wc_cog()->is_plugin_active( 'woocommmerce-bookings.php' ) ) {

			// add cost field to booking products under the 'General' tab
			add_action( 'woocommerce_product_options_sku', array( $this, 'add_cost_field_to_booking_product' ) );
	...
	public function add_variable_product_bulk_edit_cost_action() {
		...
		if ( SV_WC_Plugin_Compatibility::is_wc_version_lt_2_4() ) {

			$option_label = __( 'Cost', 'woocommerce-cost-of-goods' );

			wc_enqueue_js( '
				$( "select#field_to_edit" ).on( "variable_cost_of_good", function() {
					var value = prompt( woocommerce_admin_meta_boxes_variations.i18n_enter_a_value );
					$( "input[name^=variable_cost_of_good]" ).val( value ).change();
				} );
			' );
		}
