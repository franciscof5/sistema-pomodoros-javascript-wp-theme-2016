function frmProFormJS() {
	...
	*/
	...
	function calcProductsTotal( e ) {
		var totalFields, leftSymbol, rightSymbol;
		...
		totalFields.each( function() {
			...
			currency = getCurrency( $form.find( 'input[name="form_id"]' ).val() );
			...
			$form.find( 'input[data-frmprice],select:has([data-frmprice])' ).each( function() {
				...
				if ( this.hasAttribute( 'data-frmhigherpg' ) ) {
					return;
				}
	...
	return {
		init: function() {
			...
			// Total field calc
			jQuery( document ).on( 'change', '[type="checkbox"][data-frmprice],[type="radio"][data-frmprice],select:has([data-frmprice])', calcProductsTotal );
			jQuery( document ).on( 'keyup change', '[data-frmproduct],input[type="text"][data-frmprice]', calcProductsTotal );
