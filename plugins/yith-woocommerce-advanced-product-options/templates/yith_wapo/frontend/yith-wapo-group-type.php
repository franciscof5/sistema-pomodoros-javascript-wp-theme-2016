...
$conditional = $single_type['depend'];
...
$empty_option_text = apply_filters( 'ywapo_empty_option_text' , __( 'Select an option...' , 'yith-woocommerce-advanced-product-options' ) ) ;
...
<div id="<?php echo $value ?>" class="ywapo_group_container ywapo_group_container_<?php echo $type; ?> form-row form-row-wide <?php echo $conditional_hidden; ?>" data-requested="<?php echo $required ? '1' : '0' ; ?>" data-type="<?php echo $type; ?>" data-id="<?php echo $type_id; ?>" data-condition="<?php echo esc_attr( $conditional ) ?>">
    ...
    if( is_array( $options ) ) {
        ...
        for( $i=0; $i< count($options['label']) ; $i++ ) {
            ...
            $yith_wapo_frontend->printOptions( $i, $product, $type_id, $type, $name, $value, $options['price'][$i], $options['label'][$i], $image, $price_type,$description, $required, $checked, $disabled, 'before' , $min, $max );
