...
<p>
	<label>
		<?php esc_html_e( 'Show attendees list on event page', 'event-tickets-plus' ); ?>
		<input
			type="checkbox"
	...
	<fieldset class="<?php echo esc_attr( $fieldset_class ); ?>">
		...
		<?php else : ?>
			<section style="margin-bottom: 0;">
				...
				<p class="ticket_form_right">It looks like you have multiple ecommerce plugins active. We recommend running only one at a time. However, if you need to run multiple, please select which one to use to sell tickets for this event.</p>
				<?php foreach ( $modules as $class => $module ) : ?>
