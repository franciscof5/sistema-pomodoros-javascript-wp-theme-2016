 ...
 */
if ( !class_exists( 'TimelineSettings' ) ):
class TimelineSettings {
     ...
     * @param array   $args settings field args
     */
    function callback_textblock( $args ) {
    ...
    function callback_number( $args ) {
        ...
        $html = sprintf( '<input type="number" class="%1$s-text" id="%2$s[%3$s]" name="%2$s[%3$s]" min="1" value="%4$s"/>', $size, $args['section'], $args['id'], $value );
        $html .= sprintf( '<span class="description"> %s</span>', $args['desc'] );
    ...
    function callback_radio( $args ) {
        ...
        $html = '';
        foreach ( $args['options'] as $key => $label ) {
            $html .= sprintf( '<input type="radio" class="radio timeline-radio-input" id="%1$s[%2$s][%3$s]" name="%1$s[%2$s]" value="%3$s"%4$s />', $args['section'], $args['id'], $key, checked( $value, $key, false ) );
            $html .= sprintf( '<label class="timeline-radio-label" for="%1$s[%2$s][%4$s]"> %3$s</label>', $args['section'], $args['id'], $label, $key );
        }
        $html .= sprintf( '<span class="description"> %s</label>', $args['desc'] );
