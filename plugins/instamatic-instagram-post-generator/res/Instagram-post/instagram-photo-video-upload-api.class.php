...
class InstagramLoginPassMethod
{
	...
	public function __construct ( $username, $password, $proxy = '' )
	{
		...
		$cookies = TRUE;
		$getSettings = get_option('instamatic_cache_sess' . $username);

		if ( $getSettings )
		{
			$this->cache_data_id = 'instamatic_cache_sess' . $username;
	...
	public function __destruct ()
	{
		...
		{
			update_option('instamatic_cache_sess' . $this->username, [
	...
	public function imageForFeed ( $photo )
	{
		error_reporting(0);
		...
		if ( $ratio1 > static::MAX_ASPECT_RATIO )
		{
			$newWidth = (int) ( $height * static::MAX_ASPECT_RATIO );
		}
		else if ( $ratio1 < static::MIN_ASPECT_RATIO )
		{
			$newHeight = (int) ( $width / static::MIN_ASPECT_RATIO );
		}
		...
		$newFileName = sys_get_temp_dir() . DIRECTORY_SEPARATOR . uniqid( 'cr_' );
		global $wp_filesystem;
		if ( ! is_a( $wp_filesystem, 'WP_Filesystem_Base') ){
			include_once(ABSPATH . 'wp-admin/includes/file.php');$creds = request_filesystem_credentials( site_url() );
			wp_filesystem($creds);
		}
		if ($wp_filesystem->exists($newFileName)) {
			$wp_filesystem->delete($newFileName);
		}
	...
	private function splitVideoSegments ( $video, $target )
	{
		...
		$segmentsPath         = sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'cr_' . $segmentId . '_%03d.mp4';
		$segmentsPathForAudio = sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'cr_' . $segmentId . '_audio.mp4';
	...
	{
		$segmentsPath      = sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'cr_' . $segmentId . '_*.mp4';
		$segmentsPathAudio = sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'cr_' . $segmentId . '_audio.mp4';
