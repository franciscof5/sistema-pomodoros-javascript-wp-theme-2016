(function($) {
	...
	var lazy_to_img_url = function(subj_id, is_coll) {
		...
		$subj.each(function() {
			if(jQuery(this).data('gg-lazy-src') != 'undefined') {
				jQuery(this).attr('src', jQuery(this).data('gg-lazy-src'));
	...
	var layout_standard_gall = function(gid, has_new_img) {
		...
		jQuery('#'+gid+' .gg_img').each(function(i) {
			...
			if(row_w >= gg_gallery_w[gid] && b == img_per_row) { 
	...
	gg_man_masonry_gallery = function(gid) {
		...
		var cols = parseInt(jQuery('#'+gid).data('col-num')); 
	...
	gg_man_collection = function(cid) {
		...
		var cols = parseInt(jQuery('#'+cid).data('col-num')); 
		var margin = gg_img_margin[cid] + gg_img_margin_l[cid];
		var col_w = (gg_gallery_w[cid] / cols) - (margin * (cols - 1));
		
		
		// if min column is higher than colection's width - force 1 column
		if(gg_gallery_w[cid] <= gg_coll_min_w) {
			cols = 1;
			var col_w =	gg_gallery_w[cid] - margin;
		}
		else {
	...
	jQuery(document).ready(function() {
		jQuery('body').delegate('.gg_coll_img:not(.gg_linked_img)', 'click', function() {
			...
			var gdata = jQuery(this).data('gall-data');
	...
	gg_load_coll_gallery = function(cid, gdata) {
		...
		coll_scroll_helper[cid] = jQuery('#'+cid+' .gg_coll_img[data-gall-data="'+gdata+'"]');
	...
	function gg_get_cg_deeplink(browser_history) { // coll selection
		...
		if(hash.indexOf('#gg_cg') !== -1) {
			...
			if( jQuery('.gg_coll_img[rel=' + gid + ']').length ) {
				...
				var gdata = jQuery('.gg_coll_img[rel=' + gid + ']').first().data('gall-data');
	...
	jQuery(document).ready(function() {
		...
		jQuery(document).ready(function() {
			jQuery('body').delegate('.gg_num_btns_wrap > div', 'click', function() {
				...
				if( !jQuery(this).hasClass('gg_pag_disabled') && gg_is_paginating[gid] == 0 ){
					gg_gallery_pag[gid] = jQuery(this).attr('rel'); 
	...
	gg_standard_pagination = function(gid, new_pag) {
		if(gg_is_paginating[gid] == 0) {
			...
			if(jQuery('#'+gid+' .gg_standard_pag').length) {
				if(new_pag >= parseInt(jQuery('#'+gid+' .gg_paginate').data('gg-totpages'))) { jQuery('#'+gid+' .gg_paginate').find('.gg_next_page').addClass('gg_pag_disabled'); }
			...
			// set new pag number 
			jQuery('#'+gid+' .gg_nav_mid span').text(new_pag);
	...
	gg_galleria_height = function(sid) {
		if( jQuery(sid).hasClass('gg_galleria_responsive')) {
			return parseFloat( jQuery(sid).data('asp-ratio') );
		} else {
	...
	gg_galleria_init = function(sid) {
		// autoplay flag
		var spec_autop = jQuery(sid).data('gg-autoplay');
		...
		Galleria.run(sid, {
			...
			dataConfig: function(img) {
				return {
					...
					description: jQuery(img).data('description')
				};
			},
	...
	jQuery(window).gg_smartresize(function() {
		...
		jQuery('.gg_galleria_responsive').each(function() {	
			...
			var gg_asp_ratio = parseFloat(jQuery(this).data('asp-ratio'));
	...
	jQuery(document).ready(function() {
		jQuery(document).delegate('.gg_gallery_wrap div.gg_img:not(.gg_coll_img, .gg_linked_img)', 'click', function(e) {	
			...
			jQuery('#'+gid+' .gg_img:not(.gg_coll_img, .gg_linked_img)').each(function(i, v) {
				var img_url = jQuery(this).data('gg-url');
				
				if(typeof( gall_obj[img_url] ) == 'undefined') {
					gall_obj[img_url] = {
						"img"		: img_url,
						"title"		: gg_lb_html_fix(jQuery(this).data('gg-title')),
						"descr"		: gg_lb_html_fix(jQuery(this).data('gg-descr')),
						'author'	: gg_lb_html_fix(jQuery(this).data('gg-author'))
					};	
