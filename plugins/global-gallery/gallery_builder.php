...
function gg_gallery_type() {
	...
	$usern_vis 	= (in_array($type, array_merge($conn_hub->to_consider, array('wp', 'wp_cat', 'cpt_tax', 'gg_album','ngg', 'dropbox')))) ? 'style="display: none;"' : '';
	...
	jQuery(document).ready(function($) {
		...
		jQuery(document).delegate('#gg_type_dd', 'change', function(e) {
			if( jQuery('#gg_fb_builder').size() == 0 || ( jQuery('#gg_fb_builder').size() && confirm("<?php _e('Current gallery will be erased. Continue?', 'gg_ml') ?>") ) ) {
				...
				if( jQuery.inArray(gg_new_type, ['<?php echo  implode("','", array_merge($conn_hub->to_consider, array('wp', 'wp_cat', 'cpt_tax', 'gg_album','ngg', 'dropbox'))) ?>']) === -1 ) {
