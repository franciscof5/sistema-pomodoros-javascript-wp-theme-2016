...
function gg_gallery_shortcode( $atts, $content = null ) {
	...
	if($layout == 'masonry') { $add_param = 'data-col-num="'.$cols.'"'; }
	elseif($layout == 'string') { $add_param = 'data-row-h="'.$row_h.'"'; }
	else {$add_param = '';} 
	...
	// build
	$gallery .= '
	<div id="'.$unique_id.'" class="gg_gallery_wrap gg_'.$layout.'_gallery gid_'.$gid.' '.$ol_man->ol_wrap_class.' '.$ol_man->txt_vis_class.'" data-gg_ol="'.$overlay.'" '.$add_param.' '.$ol_man->img_fx_attr.' rel="'.$gid.'">
      '.gg_preloader().'
...
function gg_collection_shortcode( $atts, $content = null ) {
	...
	// build
	$collection .= '
	<div id="'.$unique_id.'" class="gg_gallery_wrap gg_collection_wrap cid_'.$cid.'" rel="'.$cid.'" data-col-num="'.gg_float_to_cols_num($thumb_col_w).'" '.$ol_man->img_fx_attr.'>';
	  ...
	  <table class="gg_coll_table">
		  ...
		  foreach($galleries as $gal) {
			  ...
			  <div class="gg_coll_img_wrap '.$gall_cats_list.'">
				  <div class="gg_img gg_coll_img '.$add_class.'" rel="'.$gal['id'].'" data-gall-data="'.$gal['id'].';'.$gal['rand'].';'.$gal['wmark'].'" '.$link_tag.'>
					  <div class="gg_main_img_wrap">
...
function gg_carousel_shortcode( $atts, $content = null ) {
	...
	<div id="gg_car_'.$gid.'" rel="'.$gid.'" class="gg_carousel_wrap gg_gallery_wrap gg_car_preload gg_car_'.get_option('gg_car_elem_style', 'light').' '.$has_arrows_class.' '.$pause_on_h.' '.$ol_man->txt_vis_class.' '.$ol_man->ol_wrap_class.'" '.$ol_man->img_fx_attr.'>';
	  ...
	  foreach($images as $img) {
		  ...
		  '.$open_tag.' data-gg-url="'.$img['url'].'" data-gg-title="'.gg_sanitize_input($img['title']).'" class="gg_img gg_car_item '.$add_class.'" data-gg-author="'.gg_sanitize_input($img['author']).'" data-gg-descr="'.gg_sanitize_input($img['descr']).'" rel="'.$gid.'" style="height: '.(int)$height.'px;">
			<div class="gg_img_inner">';
